...
manifest <- inputs$fit_manifest
uc_assert(file.exists(file.path(UC_ROOT, "DATA_REUSE_COMPLETE.txt")),
          "Bound observation-only data reuse is incomplete.")
for (data_id in UC_DATA_IDS) uc_load_observation(data_id)

worker_path <- file.path(UC_ROOT, "s4e_worker_20260827_v1.R")
uc_assert(file.exists(worker_path), "Stage-4E worker script is missing.")
if (check_only) {
  ...
  cat("STAGE4E_SUPERVISOR_CHECK_ONLY_PASS rows=18 workers=4 fixed_t1=400\n")
...
write_state("RUNNING", "18 registered independent fixed-400 G12 fits")
...
utils::write.csv(terminals, file.path(UC_ROOT, "ALL_18_NEW_TERMINALS.csv"),
                 row.names = FALSE, quote = TRUE, na = "")
writeLines(c(
  "status=FITS_COMPLETE_FIXED_400",
  paste0("completed_utc=", uc_iso_time()),
  "fits=18", "objective_eligible=18", "outer_workers=4",
...
write_state("FITS_COMPLETE_WAITING_FOR_TRUTH_FREE_SELECTION",
            "18/18 objective-eligible fixed-400 fits")
cat("STAGE4E_FITS_COMPLETE fixed_400=18/18 objective_eligible=18 truth=0\n")
