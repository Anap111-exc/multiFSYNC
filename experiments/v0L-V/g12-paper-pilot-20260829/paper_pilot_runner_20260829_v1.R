...
source(file.path(script_root, "paper_pilot_common_20260829_v1.R"),
       local = FALSE)
...
verify_environment <- function(include_evaluation = FALSE) {
  ...
  pp_assert(nrow(qc) == 10L && all(qc$passed), "Manifest QC is not 10/10.")
  manifests
}
...
generate_all <- function(smoke = FALSE) {
  ...
  pp_write_lines(c(
    ...
    "data_sets=2", "observation_bundles=2", "sealed_truth_bundles=2",
  ...
  cat("DATA_GENERATION_AND_SEAL_COMPLETE data=2 smoke=", smoke, "\n", sep = "")
  invisible(status)
}
...
freeze_selection <- function(expected_fits = PP_EXPECTED_FITS, smoke = FALSE) {
  ...
  manifests <- verify_environment(FALSE)
  manifest <- if (smoke) {
    keep <- ave(seq_len(nrow(manifests$fits)),
                manifests$fits$data_id, manifests$fits$method_id,
                FUN = seq_along) == 1L
    manifests$fits[keep, , drop = FALSE]
  } else manifests$fits
  ...
  expected_winners <- length(unique(manifest$data_id))
  pp_assert(nrow(winners) == expected_winners &&
              all(table(winners$data_id) == 1L),
  ...
  pp_write_lines(c(
    ...
    "winner_rule=maximum_eligible_ordinary_T1_ELBO_within_data",
...
run_supervisor <- function() {
  ...
  write_status("RUNNING", "26 registered fits; no selective rerun")
  ...
  write_status("WAITING_FOR_TRUTH_UNSEAL_AUTHORIZATION",
               "26/26 complete; two G12 winners frozen")
  invisible(TRUE)
}
...
evaluate_all <- function(smoke = FALSE) {
  ...
  bayes_environment <- candidate2_load_bayes_reference(PP_SNAPSHOT)
  ...
  for (index in seq_len(nrow(manifest))) {
    ...
    result <- tryCatch(withCallingHandlers({
      pp_evaluate_fit(
        ...
        evaluator_environment, bayes_environment
      )
    ...
    science <- if (row$method_id[[1L]] == "G12") {
    ...
    } else pp_pooled_compact_summary(result)
    compact[[index]] <- cbind(base, science)
    expected_tables <- if (row$method_id[[1L]] == "G12") c(
      ...
      "loading_scale_summary"
    ) else c(
      "direction_matches", "direction_metrics", "feature_and_score_metrics",
      "reconstruction_metrics", "contract"
    )
    ...
    checkpoint_complete <- if (row$method_id[[1L]] != "G12" || smoke) TRUE else
  ...
  pp_write_csv(all_science,
               file.path(evaluation_root, "ALL_PILOT_SCIENTIFIC_RESULTS.csv"))
  winner_ids <- pp_read_csv(file.path(
    output_root, "truth_free_selection", "G12_TRUTH_FREE_WINNERS.csv"
  ))$fit_id
  paper_endpoints <- all_science[
    all_science$fit_id %in% winner_ids |
      all_science$method_id == "pooled_bayesSYNC", , drop = FALSE
  ]
  pp_write_csv(paper_endpoints,
               file.path(evaluation_root, "PAPER_ENDPOINT_RESULTS.csv"))
  pp_write_lines(c(
    "status=EVALUATION_COMPLETE", paste0("completed_utc=", pp_iso_time()),
    paste0("fits_evaluated=", nrow(manifest)), "evaluation_errors=0",
    paste0("g12_winners=", length(winner_ids)), "pooled_endpoints=2",
    paste0("selection_sha256=", selection_hash),
...
summarize_run <- function(smoke = FALSE) {
  ...
  runtime <- do.call(rbind, lapply(split(terminal, terminal$method_id), function(x) {
    data.frame(
...
run_smoke <- function() {
  ...
  if (!nzchar(output_root)) {
    output_root <<- file.path(tempdir(), paste0("g12_paper_pilot_smoke_",
  ...
  keep <- ave(seq_len(nrow(manifests$fits)),
              manifests$fits$data_id, manifests$fits$method_id,
              FUN = seq_along) == 1L
  smoke_manifest <- manifests$fits[keep, , drop = FALSE]
  ...
  pp_write_lines(c(
    ...
    "data_sets=2", "fits=4", "g12_winners=2", "pooled_endpoints=2",
    "truth_free_selection_preceded_evaluation=TRUE",
    "formal_paper_mc_result=FALSE"
  ), file.path(output_root, "SMOKE_COMPLETE.txt"))
  cat("PAPER_PILOT_MICRO_SMOKE_PASS output_root=", output_root, "\n", sep = "")
...
if (action == "check") {
  ...
  freeze_selection(if (smoke) 4L else PP_EXPECTED_FITS, smoke)
